...
class TestSpineWiring:
    ...
    def test_quality_consumed_before_current_turn_phi(self) -> None:
        ...
        sp.process(
            "嗯",
            timestamp=1.0,
            assessment={"valence": 0.9, "arousal": 0.0, "wound_risk": 0.0, "confidence": 0.9},
        )
        scar = sp._engine.scar_state
        ...
        sp.process(
            "！！！",
            timestamp=2.0,
            assessment={"valence": 0.0, "arousal": 0.95, "wound_risk": 0.0, "confidence": 0.9},
        ...
        for i in range(8):
            if phi_after_t1[i] == 0.0:
                assert gain_after[i] == gain_before[i], f"dim{i} 在 turn1 不活跃却领了 turn1 的赏"
