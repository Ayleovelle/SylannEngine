...
DOC_PAGES = [
    {
        ...
        "src": "SPEC.md",
        "crumb": "SPEC 标准规范",
        "label": "SDK Specification · sylanne.engine.v1",
        "h1": 'SPEC<span class="em"> 规范</span>',
        "tagline": "接口协议 · 输入输出契约 · 生命周期",
    },
    {
        "name": "guide",
        "src": "AGENT_GUIDE.md",
        ...
        "h1": '开发者<span class="em">指南</span>',
        "tagline": "功能模块 · 字段含义 · 集成方法",
    },
]
...
def _mermaid_html(src):
    ...
    return '<div class="mermaid-wrap"><pre class="mermaid" data-msrc="' + esc + '"></pre></div>'
...
def _rewrite_doc_links(html):
    ...
    def repl(m):
        ...
        if path in _IN_SITE_DOC:  # 站内已嵌 -> 走 SPA 路由
            return f'href="{_IN_SITE_DOC[path]}"'
...
def md_to_html(md):
    ...
    # 3. 表格套横向滚动容器（窄屏不撑破布局）
    html = re.sub(r"(<table>.*?</table>)", r'<div class="table-wrap">\1</div>', html, flags=re.S)
    # 3.5 目录（紧跟「目录」标题的首个 ul）套 .doc-toc 卡片样式，链接更像可点的入口
    html = re.sub(
        r"(<h2[^>]*>\s*目录\s*</h2>\s*)<ul>", r'\1<ul class="doc-toc">', html, count=1, flags=re.S
    )
...
def build_html():
    parts = [
        read(f"{SRC}/partials/head.html").rstrip("\n"),
...
def stamp_versions():
    ...
    for name in MUTABLE_ASSETS:
        ...
        with open(p, "rb") as fb:
            h = hashlib.md5(fb.read()).hexdigest()[:8]
