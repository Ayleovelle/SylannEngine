...
# v26 A.1：意图直出版 prompt。仅在 affect takeover（Gate B）开启时使用——Gate A/关闭态
# 沿用上面的原 prompt（连 LLM 提示词都逐字节不变，保零行为变更契约；Gate A 下 intent
# 若通电会点燃遗留手写意图规则、打破影子期字节一致）。
_INTENT_VOCAB: tuple[str, ...] = ("撒娇", "生气", "道歉", "施压", "分享", "提问", "冷淡")

_SYSTEM_PROMPT_WITH_INTENT = _SYSTEM_PROMPT.replace(
    '    "wound_risk": 0.0-1.0\n}',
    '    "wound_risk": 0.0-1.0,\n    "intent": "撒娇|生气|道歉|施压|分享|提问|冷淡|无"\n}',
).replace(
    "- valence/arousal/wound_risk 是连续语义判断，请按文本真实含义给值，不要一律给 0",
    """- valence/arousal/wound_risk 是连续语义判断，请按文本真实含义给值，不要一律给 0
- intent: 说话者对倾诉对象的主导社交意图，从固定词表选一个：
  撒娇(亲昵求关注)/生气(指责不满)/道歉(认错求和)/施压(越界强迫)/分享(报喜炫耀)/
  提问(求助请教)/冷淡(敷衍疏离)；都不符合给 "无\"""",
)
...
async def assess_text(
    ...
    *,
    want_intent: bool = False,
) -> dict[str, Any]:
    """评估文本。``want_intent``（v26 A.1）仅在 affect takeover 开启时由引擎传 True：
    换用带 intent 字段的 prompt 并解析/消毒 intent。False（默认）下 prompt 与输出
    逐字节等同旧版——Gate A/关闭态零行为变更。"""
    if not text.strip():
        return _neutral(flags=["idle"], confidence=0.0)

    try:
        prompt = _SYSTEM_PROMPT_WITH_INTENT if want_intent else _SYSTEM_PROMPT
        response = await llm(prompt, text)
        return _parse_response(response, want_intent=want_intent)
    except Exception as e:
        logger.debug("Assessor LLM call failed: %s", e)
        result = _local_fallback(text, want_intent=want_intent)
...
def _sanitize_intent(raw: Any) -> str:
    """v26 A.1：intent 消毒——**精确匹配**固定词表（prompt 要求裸标签，只剥空白与
    首尾引号/标点），其余（含"无"/否定式如"不生气"/空/非串/超长垃圾）一律归空串
    （= 今日生产语义，不触发任何意图路径）。红队修订：此前的子串包含匹配会把
    "不生气"→生气、"是生气不是撒娇"→撒娇（词表序抢占）——否定/转述全军覆没。"""
    if not isinstance(raw, str):
        return ""
    s = raw.strip()[:24].strip("\"'。，！？!?.,、：:; 	")
    return s if s in _INTENT_VOCAB else ""


def _parse_response(response: str, *, want_intent: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        # v26 A.1：仅在 want_intent（takeover 开）时输出 intent 键——关闭态输出
        # 字典键集与旧版逐字相同。
        if want_intent:
            out["intent"] = _sanitize_intent(data.get("intent"))
        return out
...
# v26 A.1 红队修订：本地兜底**不做**意图分类——对用户原文跑关键词分类会把否定/疑问
# 误判成表达（"你生气了吗"→生气、"我没有生气"→生气），而 intent 直驱 E 律 appraisal
# 偏置、兜底的低 confidence 并不衰减它。真语义判别是 LLM 的活；LLM 不在场就诚实给空
# （= 不触发任何意图路径），与本模块"兜底刻意粗糙"的既定纪律一致。
def _local_fallback(text: str, *, want_intent: bool = False) -> dict[str, Any]:
