...
def _sanitize_intent(raw: Any) -> str:
    """v26 A.1：intent 消毒——只放行固定词表（含在串中即命中，前缀优先序同词表），
    其余（含"无"/空/非串/超长垃圾）一律归空串（= 今日生产语义，不触发任何意图路径）。"""
    if not isinstance(raw, str):
        return ""
    s = raw.strip()[:24]
    for label in _INTENT_VOCAB:
        if label in s:
            return label
    return ""
...
# v26 A.1：本地兜底的意图分类——直接复用 affect_projection 的关键词分类器对原文分类，
# 再映射回固定中文词表（两个消费方——遗留手写规则与 project_appraisal——都认这些标签）。
_CLASS_TO_LABEL: dict[str, str] = {
    "coax": "撒娇",
    "anger": "生气",
    "apologize": "道歉",
    "press": "施压",
...
def _fallback_intent(text: str) -> str:
    from .compute.affect_projection import classify_intent

    return _CLASS_TO_LABEL.get(classify_intent(text) or "", "")
...
def _local_fallback(text: str, *, want_intent: bool = False) -> dict[str, Any]:
    ...
    if want_intent:
        out["intent"] = _fallback_intent(text)
    return out
