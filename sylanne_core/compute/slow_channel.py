...
class SlowChannel:
    ...
    def observe(self, a_k: list[float]) -> None:
        ...
        for trait, mapping in _TRAIT_DRIFT_MAP.items():
            # gemini review：消毒非有限 a_k 分量——否则 NaN/inf 会顺 contrib 灌进
            # _pending 漂移累积器、污染持久人格特质（与 poignancy_magnitude 的守卫一致）。
            contrib = sum(
                w * (a_k[d] if d < len(a_k) and math.isfinite(a_k[d]) else 0.0) for d, w in mapping
            )
