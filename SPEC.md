...
| `state` | `(session_id: str) -> Surface` | 查询当前状态（不触发计算） |
| `reset` | `(session_id: str) -> None` | 重置会话状态 |
| `destroy` | `(session_id: str) -> None` | 销毁会话及持久化数据 |

### 2.3 Context Parameters / 上下文参数 (`**ctx`)

| Parameter / 参数 | Type / 类型 | Default / 默认值 | Description / 说明 |
|-----------|------|---------|-------------|
| `confidence` | `float` | `0.0` | 语义置信度 [0, 1]，0 表示由内部 assessor 计算 |
...
- **Memory consolidation / 记忆整合**：记忆层级提升评估（L1→L2）
...
```jsonc
{
    "recalled": [                          // 召回的记忆列表
        {
            "text": "string",              // 记忆内容
            "relevance": 0.85,             // 相关度 [0, 1]
            "created_at": 1716900000.0,    // 创建时间戳
            "layer": "L1"                  // 记忆层级：L1(热) / L2(温) / L3(冷)
        }
    ],
    "total_stored": 42                     // 当前会话总记忆条数
}
```
...
| Embedding unavailable / 向量化不可用 | 使用关键词匹配召回记忆 |
...
class SylanneConfig:
    ...
    memory_capacity: int = 500         # 单会话最大记忆条数
