...
assert sylanne_core.__version__.split(".")[0] == "2", (
    f"sylanne-core {sylanne_core.__version__} incompatible; need 2.x"
)
```
...
class SylanneConfig:
    ...
    affect_dynamics_enabled: bool = False  # 情感动力学 E 律**影子**（实验性，Gate A，见 §7.2）
    affect_takeover: bool = False          # E 律**夺权**写 base（实验性，Gate B；需 affect_dynamics_enabled，与 pel_core_enabled 互斥）
    affect_slowchannel_enabled: bool = False  # 慢通道 poignancy→反思→宏漂移（实验性，Gate C）
    affect_plasticity_enabled: bool = False   # delta-rule 增益可塑性（实验性，Gate B+；需 takeover）
    affect_full_takeover: bool = False        # E 律全权：主步 MLP 旁路（实验性，Gate B-full；需 takeover）
...
### 7.2 Affect-Dynamics / 情感动力学（实验性，分阶段，默认全关）

在 8 维情感核（`lite` 档）上引入双速情感动力学 E 律（设计代号 v26，见 `docs/design/`；版本号待发布时定）：墙钟惰性衰减到人格均衡 Φ_eq（慢/时间通道）+ 每轮 appraisal 饱和快更新（快/语义通道）。**三个开关默认全 `False`，全关时行为与 2.5 字节一致**（固定输入下确定性字段逐字相同、快照无新增键）；按闸位分阶段开启：

| 开关 | 闸位 | 语义 | 前置约束 |
|---|---|---|---|
| `affect_dynamics_enabled` | Gate A | E 律**只算不写**：并行计算一个"影子"E（衰减+快更新），落 debug 日志与诊断，**绝不写 `base`、不进 `observe()`、不进 prompt** | — |
| `affect_takeover` | Gate B | E 律**夺权**：衰减在 `step()` 顶部写权威 `base`（事件演化之前），饱和快更新替代 assessor 手写意图规则；任一环节异常**fail-closed 回落旧规则** | 需 `affect_dynamics_enabled=True`；**与 `pel_core_enabled` 互斥**（二者都主张 base 演化，同开将报错） |
| `affect_slowchannel_enabled` | Gate C | 慢通道：poignancy 漏桶 → 墙钟冷却反思 → 锚回弹宏漂移（不可逆记忆写，含版本化回滚环） | — |
| `affect_plasticity_enabled` | Gate B+ | delta-rule 增益学习：G ← Π_{[ε,1]}(G+α·δ·φ)，滞后 quality 反馈驱动，资格迹信用分配；安全全由投影承担 | 需 `affect_takeover` |
| `affect_full_takeover` | Gate B-full | E 律全权：主步 MLP/PEL 演化在 8 维核旁路，base 只由衰减+appraisal+伤痕形成驱动——可观测常驻情绪=Φ_eq、半衰期成为活杠杆；非 assessor 创伤通道（Γ 耦合/feedback）变哑（伤痕照常形成） | 需 `affect_takeover` |

约束由 `SylanneConfig.__post_init__` 校验（违背即 `ValueError`）。开启 Gate B/C 属**有意的行为变更**（非字节一致），其验收门是 warmth 行为标定——标定完成前不建议在生产开启。

**关系相位 API**：`await engine.set_relationship(session_id, r)`——host 显式供给 R∈[0,1]
（"处到哪一步了"语义由宿主定义），Φ_eq 的 warmth 行随 R 上移（关系越深、常驻越暖）；
越界抛 `ValueError`；随快照持久化；未调用保持 0.5（今日行为）。仅 affect 启用时有可观测效果。

启用 `affect_dynamics_enabled`（或更高闸位）且 `diagnostics=True` 时，`diagnostics` 面板附一个只读键 `affect_label_shadow`（circumplex 情绪标签，如 `平静`/`愉悦`/`焦躁`，带迟滞抗抖）——**仅诊断**，不进 prompt，与既有 `Surface.pad.label` 并存（是否取代待后续版本定）。关闭时该键不出现（诊断面板字节一致）。
